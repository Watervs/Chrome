...
-- EARLY CONFIG LOAD
-- ------------------------------------------------------------
M.introSoundEnabled = false -- disabled
M.introSongChoice = 3
M.introGUIEnabled = true
-- Fixed intro track (Chrome.VS / CHROME 2.1 style) — not changeable
M.INTRO_MUSIC_URL = "https://files.catbox.moe/qhpfe5.mp3"
M.INTRO_MUSIC_FILE = "ChromeIntro_Music.mp3"
introSoundInstance = nil

-- ============================================================
-- CHROME.VS EXECUTOR PFP — auto-store on load
-- ============================================================
M.CHROMEVS_PFP_URL  = "https://litter.catbox.moe/0mlyf2.jpg"
M.CHROMEVS_PFP_FILE = "chrome.vs.png"
M.CHROMEVS_PFP_ALT  = "pfp.png"
M.CHROMEVS_PFP_ALT2 = "exec_pfp.png"

-- Chrome avatar / executor PFP download removed
task.spawn(function()
    -- disabled
end)

-- ============================================================
-- DEFAULT GUI BACKGROUND (Discord icon)
-- ============================================================
M.CUSTOM_BG_URL  = "https://cdn.discordapp.com/icons/1536210269783457843/b1086a34d3d31ff2ba5fa90ab6a81692.webp?size=1536"
M.CUSTOM_BG_FILE = "chrome_duels_bg.webp"
M.customBgAsset  = nil -- will be set to getcustomasset result
...
task.spawn(function()
    ...
    local function alreadyGood(name)
        ...
        if type(readfile) == "function" then
            ...
            if rok and content and #content > 2000 then return true end
            return false
        end
        return true
    end
    -- always refresh or load
    local data
    local ok, res = pcall(function() return game:HttpGet(M.CUSTOM_BG_URL) end)
    if ok and type(res) == "string" and #res > 2000 then
        data = res
        tryWrite(M.CUSTOM_BG_FILE, data)
    elseif alreadyGood(M.CUSTOM_BG_FILE) then
        -- already on disk
    else
        return
    end
    pcall(function()
        if getcustomasset then
            M.customBgAsset = getcustomasset(M.CUSTOM_BG_FILE)
        end
    end)
end)
...
-- Force disabled (user request)
M.introSoundEnabled = false
M.chromeBlackSkinEnabled = false
...
M.animPack = "Bubbly" -- girl-style default for CHROME users
M.animPackEnabled = true
M.savedAnimate = nil
M.chromeBlackSkinEnabled = false -- disabled (no chrome avatars)
...
function M.applyCharterToChar(char)
    ...
    if M.chromeBlackSkinEnabled then
        M.applyChromeBlackSkin(char)
    end
end

-- Full black body + no accessories/clothes + CHROME torso chip (visible all sides)
function M.stripChromeClothing(char)
    if not char then return end
    for _, c in ipairs(char:GetChildren()) do
        if c:IsA("Accessory") or c:IsA("Hat") or c:IsA("Shirt") or c:IsA("Pants")
            ...
            if not c:GetAttribute("ChromeChip") then
    ...
    if hum then
        pcall(function()
            for _, a in ipairs(hum:GetAccessories()) do
                if not a:GetAttribute("ChromeChip") then pcall(function() a:Destroy() end) end
            end
        end)
        for _, d in ipairs(hum:GetDescendants()) do
            if d:IsA("Accessory") or d.ClassName == "LayeredClothing" then
                if not d:GetAttribute("ChromeChip") then pcall(function() d:Destroy() end) end
            end
        end
    end
...
function M.attachChromeTorsoChip(char)
    if not char then return end
    local torso = char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso")
    if not torso then return end
    -- remove old chips
    for _, c in ipairs(torso:GetChildren()) do
        if c.Name == "ChromeTorsoChipBB" or c:GetAttribute("ChromeChip") then
            pcall(function() c:Destroy() end)
        end
    end
    ...
    for _, face in ipairs({Enum.NormalId.Front, Enum.NormalId.Back}) do
        ...
        sg.Name = "ChromeTorsoChipSG"
        sg:SetAttribute("ChromeChip", true)
        sg.Face = face
...
function M.applyChromeBlackSkin(char)
    char = char or player.Character
    if not char then return end
    if not M.chromeBlackSkinEnabled then return end

    M.stripChromeClothing(char)
    ...
    M.attachChromeTorsoChip(char)

    -- keep stripping clothes that get re-added
    if M._chromeBlackChildConn then
        pcall(function() M._chromeBlackChildConn:Disconnect() end)
        M._chromeBlackChildConn = nil
    end
    M._chromeBlackChildConn = char.ChildAdded:Connect(function(c)
        if not M.chromeBlackSkinEnabled then return end
        if c:GetAttribute("ChromeChip") then return end
        if c:IsA("Accessory") or c:IsA("Hat") or c:IsA("Shirt") or c:IsA("Pants")
...
M.vyncSkinEnabled = false -- disabled
...
local function vyncLoad(id)
    ...
    for _, o in ipairs(bag) do
        ...
        if acc then
            local cl = acc:Clone()
            cl:SetAttribute("ChromeSkin", true)
            cl.Name = "ChromeSkin"
...
function M.applyVyncSkin(char)
    ...
    vyncStrip(char)
    for _, c in ipairs(char:GetChildren()) do
        if (c:IsA("Accessory") or c:IsA("Hat")) and c:GetAttribute("ChromeSkin") then return end
    end
    ...
    pcall(function()
        ...
        for _, c in ipairs(char:GetChildren()) do
            if c:IsA("Accessory") or c:IsA("Hat") then
                if not kept then
                    kept = true
                    c:SetAttribute("ChromeSkin", true)
                    c.Name = "ChromeSkin"
                else
                    pcall(function() c:Destroy() end)
                end
...
function M.clearVyncSkin(char)
    ...
    if not char then return end
    for _, c in ipairs(char:GetChildren()) do
        if (c:IsA("Accessory") or c:IsA("Hat")) and (c:GetAttribute("ChromeSkin") or c.Name == "ChromeSkin") then
            pcall(function() c:Destroy() end)
        end
    end
end
...
M.antiAntiDesyncEnabled = false
M.antiTPBatEnabled = false
M.antiTPBatRange = 25
M.antiTPBatDebounce = false
...
M.autoSwitchSpeedEnabled = true -- enabled by default for Chrome Duels
...
M.stealMode = "V2" -- default: auto-steal that pauses at 75%
M.stealBarSize = 460
M.Steal = {
...
M.highlightESPEnabled = true -- outlines on other players by default
M.menuOpen = true
...
function M.setupHeadIndicator(char)
    ...
    discordLbl.Text = "chrome.vs duels"
    ...
    speedUpdateConn = RunService.Heartbeat:Connect(function(dt)
        ...
        if char and M.headIndicator and M.headIndicator.speed then
            ...
            if M.headIndicator.discord and M.headIndicator.discord.Text == "" then
                M.headIndicator.discord.Text = "chrome.vs duels"
            end
        end
    end)
end
...
-- ============================================================
-- Chrome STATUS UI (Steal Bar)
-- ============================================================

-- Steal bar green splash background (from reference HUD)
local function _chromeB64decode(data)
...
function M.buildStatusUI()
    ...
    for _, v in ipairs(gui.Parent:GetChildren()) do
        if v ~= gui and v:IsA("ScreenGui") and (v.Name == gui.Name or v.Name == "ChromeStatusUI" or v.Name == "K7_StatusUI") then
    ...
    do
        local asset = M.customBgAsset
        if (not asset or asset == "") and (tonumber(M.customBgId) or 0) > 0 then
            asset = "rbxassetid://" .. tostring(M.customBgId)
        end
...
-- ============================================================
-- ANTI ANTI-DESYNC
-- Keeps client position authority against server anti-desync
-- ============================================================
M._antiAntiDesyncConn = nil
function M.startAntiAntiDesync()
    if M._antiAntiDesyncConn then return end
    M._antiAntiDesyncConn = RunService.Heartbeat:Connect(function()
        if not M.antiAntiDesyncEnabled then return end
        local char = player.Character
        if not char then return end
        local hrp = char:FindFirstChild("HumanoidRootPart")
        local hum = char:FindFirstChildOfClass("Humanoid")
        if not hrp or not hum or hum.Health <= 0 then return end
        -- Re-assert network ownership + small velocity lock to fight anti-desync
        pcall(function()
            if hrp:CanSetNetworkOwnership() then
                hrp:SetNetworkOwner(player)
            end
        end)
        -- Keep horizontal velocity stable (prevents forced resync rubberbanding)
        local vel = hrp.AssemblyLinearVelocity
        if vel.Magnitude > 0.5 and vel.Magnitude < 200 then
            hrp.AssemblyLinearVelocity = Vector3.new(vel.X, vel.Y, vel.Z)
        end
        -- Light CFrame reaffirmation (very small, avoids visual jitter)
        if not M.isStealing and not M.laggerCarryActive then
            pcall(function()
...
function M.stopAntiAntiDesync()
    if M._antiAntiDesyncConn then
        pcall(function() M._antiAntiDesyncConn:Disconnect() end)
        M._antiAntiDesyncConn = nil
    end
end

function M.setAntiAntiDesync(on)
    M.antiAntiDesyncEnabled = on == true
    if M.antiAntiDesyncEnabled then
        M.startAntiAntiDesync()
    else
        M.stopAntiAntiDesync()
    end
    if M.setAntiAntiDesyncVisual then pcall(function() M.setAntiAntiDesyncVisual(M.antiAntiDesyncEnabled) end) end
    pcall(function() if saveCherryConfig then saveCherryConfig() end end)
end

-- ============================================================
-- ANTI TP BAT
-- Detect enemy TP bat → use OUR bat (from script) to send them to void / reset
-- ============================================================
M._antiTPBatConn = nil
local TP_BAT_NAMES = {
    "tp bat", "tpbat", "teleport bat", "tele bat", "t.p bat", "tp slap",
    "void bat", "portal bat", "warp bat", "blink bat"
}

local function isTPBatTool(tool)
    if not tool or not tool:IsA("Tool") then return false end
    local n = tool.Name:lower()
    for _, key in ipairs(TP_BAT_NAMES) do
        if n:find(key, 1, true) then return true end
    end
    if n:find("tp", 1, true) and n:find("bat", 1, true) then return true end
    if n:find("tele", 1, true) and (n:find("bat", 1, true) or n:find("slap", 1, true)) then return true end
    return false
end

local function findOurTPBat()
    local char = player.Character
    if not char then return nil end
    local function score(t)
        if not t or not t:IsA("Tool") then return -1 end
        local n = t.Name:lower()
        if isTPBatTool(t) then return 3 end
        if n:find("bat", 1, true) then return 2 end
        if n:find("slap", 1, true) then return 1 end
        return -1
    end
...
local function useOurBatOnTarget(targetChar)
    ...
    -- Equip
    if bat.Parent ~= myChar then
        pcall(function()
            if myHum then myHum:EquipTool(bat) end
        end)
        task.wait(0.05)
    end

    -- Aggressive activate + remotes (how most TP bats register hits)
    for _ = 1, 6 do
    ...
    -- Extra void attempt on target (client-side, may help on loose servers)
    pcall(function()
        tHRP.AssemblyLinearVelocity = Vector3.new(0, -800, 0)
        tHRP.CFrame = CFrame.new(0, -600, 0)
    end)
    local tHum = targetChar:FindFirstChildOfClass("Humanoid")
    if tHum then pcall(function() tHum.Health = 0 end) end
end

function M.startAntiTPBat()
    if M._antiTPBatConn then return end
    M._antiTPBatConn = RunService.Heartbeat:Connect(function()
        if not M.antiTPBatEnabled or M.antiTPBatDebounce then return end
        local myChar = player.Character
        if not myChar then return end
        local myHRP = myChar:FindFirstChild("HumanoidRootPart")
        if not myHRP then return end
        local range = tonumber(M.antiTPBatRange) or 35
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            if tool and isTPBatTool(tool) then
                holdingTP = true
            else
                local bp = plr:FindFirstChild("Backpack")
                if bp then
                    for _, t in ipairs(bp:GetChildren()) do
                        if isTPBatTool(t) then holdingTP = true break end
                    end
                end
            end

            if holdingTP then
                M.antiTPBatDebounce = true
                task.spawn(function()
                    useOurBatOnTarget(char)
                    task.wait(1.5)
                    M.antiTPBatDebounce = false
                end)
                break
...
function M.stopAntiTPBat()
    if M._antiTPBatConn then
        pcall(function() M._antiTPBatConn:Disconnect() end)
        M._antiTPBatConn = nil
    end
    M.antiTPBatDebounce = false
end

function M.setAntiTPBat(on)
    M.antiTPBatEnabled = on == true
    if M.antiTPBatEnabled then
        M.startAntiTPBat()
    else
        M.stopAntiTPBat()
    end
    if M.setAntiTPBatVisual then pcall(function() M.setAntiTPBatVisual(M.antiTPBatEnabled) end) end
    pcall(function() if saveCherryConfig then saveCherryConfig() end end)
end

-- ============================================================
-- DUEL FACE (look at opponent)
-- Only 1 other player detected → always face them
-- Multiple players → don't face anyone
-- ============================================================
M.duelFaceEnabled = true
M._duelFaceConn = nil
M._duelFaceTarget = nil

function M.startDuelFace()
    if M._duelFaceConn then return end
    M._duelFaceConn = RunService.RenderStepped:Connect(function()
        if not M.duelFaceEnabled then return end
        local myChar = player.Character
        if not myChar then return end
        local myHRP = myChar:FindFirstChild("HumanoidRootPart")
        local myHum = myChar:FindFirstChildOfClass("Humanoid")
        ...
        -- Count alive other players with characters
        local others = {}
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= player then
                ...
                if c and h and r and h.Health > 0 then
                    table.insert(others, {plr = plr, hrp = r})
                end
            end
        end

        if #others == 1 then
            local target = others[1].hrp
            M._duelFaceTarget = others[1].plr
            -- Face them (character only, keep camera free unless shiftlock)
            pcall(function()
                myHum.AutoRotate = false
                local lookPos = Vector3.new(target.Position.X, myHRP.Position.Y, target.Position.Z)
                if (lookPos - myHRP.Position).Magnitude > 0.5 then
                    myHRP.CFrame = CFrame.lookAt(myHRP.Position, lookPos)
                end
            end)
        else
            -- 0 or 2+ players: stop forcing look
            M._duelFaceTarget = nil
            pcall(function()
                if myHum and not M.autoBatEnabled and not M.bypassAimbotEnabled then
...
function M.stopDuelFace()
    if M._duelFaceConn then
        pcall(function() M._duelFaceConn:Disconnect() end)
        M._duelFaceConn = nil
    end
    M._duelFaceTarget = nil
    local char = player.Character
...
function M.setDuelFace(on)
    M.duelFaceEnabled = on == true
    if M.duelFaceEnabled then
        M.startDuelFace()
    else
        M.stopDuelFace()
    end
    if M.setDuelFaceVisual then pcall(function() M.setDuelFaceVisual(M.duelFaceEnabled) end) end
    pcall(function() if saveCherryConfig then saveCherryConfig() end end)
end
...
function M.getNormalAimbotSpeed()
    -- Chrome/S2hub: Lagger modes use lagger aimbot speed, else normal aimbot speed
...
function M.startBatAimbot()
    ...
    pcall(function() RunService:UnbindFromRenderStep("ChromeAimbotRotCam") end)
...
function M.stopBypassAimbot()
    ...
    pcall(function() RunService:UnbindFromRenderStep("ChromeBypassRotCam") end)
...
function M.startHoldInfJump()
    ...
    M.holdInfJumpConn = RunService.Heartbeat:Connect(function()
        ...
        -- Hold logic from original Chrome: continuous Velocity boost while Space/Jump held
...
function M.hasBrainrotInHand()
    ...
    for _, item in ipairs(char:GetChildren()) do
        if item:IsA("Tool") then
            if M.isBrainrotTool and M.isBrainrotTool(item) then return true end
            local name = item.Name:lower()
            if name:find("brainrot", 1, true) or name:find("skibidi", 1, true) or name:find("toilet", 1, true)
                or name:find("animal", 1, true) or name:find("carry", 1, true) then
                return true
            end
        end
    end
...
-- Drop Brainrot (fixed: unequip held brainrot + reliable ground drop)
M.lastDropTime = M.lastDropTime or 0
M.dropConnections = M.dropConnections or {}
M.DROP_ASCEND_DURATION = M.DROP_ASCEND_DURATION or 0.28
M.DROP_ASCEND_SPEED = M.DROP_ASCEND_SPEED or 140

function M.isBrainrotTool(tool)
    if not tool or not tool:IsA("Tool") then return false end
    local n = tool.Name:lower()
    return n:find("brainrot", 1, true)
        or n:find("skibidi", 1, true)
        or n:find("toilet", 1, true)
        or n:find("animal", 1, true)
        or n:find("pet", 1, true)
        or n:find("carry", 1, true)
        or n:find("hold", 1, true)
        or n:find("grab", 1, true)
        or tool:GetAttribute("Brainrot") == true
        or tool:GetAttribute("IsBrainrot") == true
end

function M.unequipBrainrotTools()
    local char = player.Character
    if not char then return false end
    local hum = char:FindFirstChildOfClass("Humanoid")
    ...
    local dropped = false
    for _, item in ipairs(char:GetChildren()) do
        if item:IsA("Tool") and M.isBrainrotTool(item) then
            pcall(function()
                if hum then hum:UnequipTools() end
            end)
            -- force parent to backpack if still on character
            if item.Parent == char and bp then
                pcall(function() item.Parent = bp end)
            end
            dropped = true
        end
    end
    -- also unequip any equipped tool (some games name brainrots differently)
    if not dropped and hum then
        for _, item in ipairs(char:GetChildren()) do
            if item:IsA("Tool") then
...
function M.runDrop()
    ...
    local now = tick()
    if M._lastDropInvoke and (now - M._lastDropInvoke) < 0.25 then return end
    M._lastDropInvoke = now

    -- only block during active steal progress, not just bypass toggle
    ...
    -- 1) Unequip / drop held brainrot tools first
    pcall(M.unequipBrainrotTools)

    M.dropActive = true
    local startTime = tick()
    local ascendDur = tonumber(M.DROP_ASCEND_DURATION) or 0.28
    local upSpeed = tonumber(M.DROP_ASCEND_SPEED) or 140
    local dropConn
    ...
    dropConn = RunService.Heartbeat:Connect(function()
        ...
        local elapsed = tick() - startTime
        if elapsed < ascendDur then
            -- push up (keep some horizontal so it feels natural)
            local v = currentRoot.AssemblyLinearVelocity
            pcall(function()
                currentRoot.AssemblyLinearVelocity = Vector3.new(v.X * 0.4, upSpeed, v.Z * 0.4)
            end)
            return
        end
        ...
        local origin = currentRoot.Position + Vector3.new(0, 5, 0)
        local rayResult = workspace:Raycast(origin, Vector3.new(0, -4000, 0), rayParams)
        -- fallback: try from higher
        if not rayResult then
            rayResult = workspace:Raycast(currentRoot.Position + Vector3.new(0, 50, 0), Vector3.new(0, -5000, 0), rayParams)
        end

        if rayResult then
            local hip = (currentHum and currentHum.HipHeight) or 2
            local offset = hip + (currentRoot.Size.Y / 2) + 0.15
            local _, yRot, _ = currentRoot.CFrame:ToEulerAnglesYXZ()
            pcall(function()
            ...
            -- no floor found: just kill vertical velocity
            pcall(function()
                local v = currentRoot.AssemblyLinearVelocity
                currentRoot.AssemblyLinearVelocity = Vector3.new(v.X, 0, v.Z)
            end)
        end

        -- ensure tools unequipped after land
        pcall(M.unequipBrainrotTools)
        M.dropActive = false
    end)
    M._dropConn = dropConn
...
M.customBgId = 0
M.customBgOpacity = 0.55 -- Discord icon bg
M.mobBtnBgId = 0
M.BG_IMAGE_IDS = {
    126860692354524,
    88369503310562,
...
local function loadCherryConfig()
    ...
    if ok and type(d)=="table" then
        ...
        if d.antiAntiDesync~=nil then M.antiAntiDesyncEnabled=d.antiAntiDesync end
        if d.duelFace~=nil then M.duelFaceEnabled=d.duelFace end
        ...
        if d.chromeBlackSkinEnabled~=nil then M.chromeBlackSkinEnabled=d.chromeBlackSkinEnabled==true end
...
local function saveCherryConfig()
    ...
    local cfg = {
        ...
        medusaCounter=M.medusaCounterEnabled, batCounter=M.batCounterEnabled, antiTPBat=M.antiTPBatEnabled, antiAntiDesync=M.antiAntiDesyncEnabled, duelFace=M.duelFaceEnabled,
        ...
        chromeBlackSkinEnabled=M.chromeBlackSkinEnabled~=false,
...
function M.applyCustomBackground(frame)
    ...
    -- Apply Discord icon as GUI background when available
    local asset = M.customBgAsset
    if not asset or asset == "" then
        -- fallback to numeric id if set
        local bgId = tonumber(M.customBgId) or 0
        if bgId > 0 then
            asset = "rbxassetid://" .. tostring(bgId)
        end
    end
    ...
    bg.BackgroundTransparency = 1
    bg.Image = asset
    bg.ImageTransparency = math.clamp(tonumber(M.customBgOpacity) or 0.45, 0.15, 0.85)
    bg.ScaleType = Enum.ScaleType.Crop
    bg.Size = UDim2.fromScale(1, 1)
    bg.ZIndex = 0
    bg.Parent = frame
...
function M.openImagePicker(kind)
    ...
    local old = player.PlayerGui:FindFirstChild("ChromeImagePicker")
    if old then old:Destroy() end
    local cg = game:GetService("CoreGui"):FindFirstChild("ChromeImagePicker")
    if cg then cg:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "ChromeImagePicker"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
function M._fontSetupCoding()
    ...
    local ok = pcall(function()
        if isfile and writefile and getcustomasset then
            if not isfile("chrome_starborn.ttf") then
                writefile("chrome_starborn.ttf", game:HttpGet("https://granny.anondrop.net/uploads/6c2505542959f371/Starborn.ttf"))
            end
            writefile("chrome_starborn.json", HS:JSONEncode({
                name = "Starborn",
                faces = {{name = "Regular", weight = 400, style = "normal", assetId = getcustomasset("chrome_starborn.ttf")}}
            }))
            M._fontMy = Font.new(getcustomasset("chrome_starborn.json"))
        end
    end)
...
-- Chrome Ping Lagger (panel UI + logic from ChromePingLagger)
-- ============================================================
local PING_BG_IDS = {
    "126860692354524",
    "88369503310562",
...
function M.setPingActive(state, isManual)
    ...
    if M.pingActive then
        ...
        if M.bypassActive then
            pcall(function()
                if M.setChromeBypassActive then M.setChromeBypassActive(false) end
            end)
        end
...
function M.buildPingLaggerUI()
    -- REMOVED: Ping Lagger permanently disabled
    return
end
--[[ ORIGINAL buildPingLaggerUI REMOVED 
    if M.pingGui then
    ...
    for _, kid in ipairs(parent:GetChildren()) do
        if kid.Name == "ChromePingLaggerGui" or kid.Name == "GalaxyPingLaggerGui" then
            pcall(function() kid:Destroy() end)
        end
    end

    local screen = Instance.new("ScreenGui")
    screen.Name = "ChromePingLaggerGui"
    ...
    titleLbl.Text = "CHROME.VS PING LAGGER"
    ...
    M._pingInputConn = UIS.InputBegan:Connect(function(input, processed)
        ...
        local isGp = chromePingIsGamepad(kc)
        local isKb = input.UserInputType == Enum.UserInputType.Keyboard
        local kbEnum = chromePingResolveKb(M.pingKeybindKb)
        local gpEnum = chromePingResolveKb(M.pingKeybindGp)
...
-- ============================================================
-- CHROME BYPASS PANEL (open/close from Utility)
-- ============================================================
local BYPASS_DEPTH = 296
local BYPASS_SPAM_DELAY = 0.12

local function _chromeBuildBomb(power)
    local maintable = {}
    local spammedtable = {}
...
function M.stopChromeBypassSpam()
    M._bypassSpamRunning = false
...
function M.startChromeBypassSpam(power)
    M.stopChromeBypassSpam()
    M._bypassSpamRunning = true
    M._bypassBomb = _chromeBuildBomb(power or M.bypassPower or 97000)
...
function M.createBypassBillboard()
    -- Head label for Chrome Bypass removed
    M.destroyBypassBillboard()
end

function M.setChromeBypassActive(state)
    if state then
    ...
    if M.bypassActive then
        M.startChromeBypassSpam(M.bypassPower or 97000)
    else
        M.stopChromeBypassSpam()
    end
...
function M.buildChromeBypassUI()
    -- REMOVED: Bypass panel permanently disabled
    return
end
--[[ ORIGINAL buildChromeBypassUI REMOVED 
    ...
    actBtn.Activated:Connect(function()
        M.setChromeBypassActive(not M.bypassActive)
        pcall(saveCherryConfig)
    end)
        ...
        char.ChildAdded:Connect(function(child)
            ...
            if child:IsA("Tool") then
                ...
                if string.find(name, "brainrot", 1, true) or string.find(name, "skibidi", 1, true) or string.find(name, "toilet", 1, true) then
                    if not M.bypassActive then M.setChromeBypassActive(true) end
                end
            end
        end)
        task.defer(function()
...
function M.buildGui()
    ...
    for _,n in ipairs({"MoveeDuels","Cherry_Menu","K7HubGUI","VantaHubUI","ChromeHubUI","ChromeHubUI","AceDuelsAdaptReconstruct"}) do
    ...
    -- Bottom of sidebar: CHROME 3 + avatar circle
    ...
    M._keybindCaptureConn = UIS.InputBegan:Connect(function(input, gameProcessed)
        ...
        elseif kbMatch(M.KB.DropBrainrot, kc) then
            ...
            if M.KB.DropBrainrot and (M.KB.DropBrainrot.kb or M.KB.DropBrainrot.gp) then
                if M.runDropBrainrot then M.runDropBrainrot() else M.runDrop() end
            end
    ...
    -- Anti TP Bat: if enemy uses TP/Teleport bat near you → send THEM to void
    local _, setAntiTPBat = uiToggleRow(PMech, "Anti TP Bat", M.antiTPBatEnabled, function(on)
        M.setAntiTPBat(on)
    end)
    M.setAntiTPBatVisual = setAntiTPBat

    -- Anti Anti-Desync: fight server anti-desync
    local _, setAntiAntiDesync = uiToggleRow(PMech, "Anti Anti Desync", M.antiAntiDesyncEnabled, function(on)
        M.setAntiAntiDesync(on)
    end)
    M.setAntiAntiDesyncVisual = setAntiAntiDesync

    local _, setDuelFace = uiToggleRow(PMech, "Duel Face (1v1 only)", M.duelFaceEnabled, function(on)
        M.setDuelFace(on)
    end)
    M.setDuelFaceVisual = setDuelFace
    ...
    local _, setVyncSkin = uiToggleRow(PUtil, "Chrome.vs Skin", M.vyncSkinEnabled == true, function(on)
        M.setVyncSkin(on)
    end)
    ...
    local _, setBlackSkin = uiToggleRow(PUtil, "Chrome.vs Black Skin", M.chromeBlackSkinEnabled ~= false, function(on)
        M.chromeBlackSkinEnabled = on
        if on and player.Character then M.applyChromeBlackSkin(player.Character) end
        saveCherryConfig()
    end)
    ...
    local _, setKillLagger = uiToggleRow(PUtil, "Chrome.vs Lagger", M.killLaggerOpen ~= false, function(on)
    ...
    local _, setPingPanel = uiToggleRow(PUtil, "Chrome.vs Ping Lagger", M.pingPanelOpen == true, function(on)
    ...
    local _, setBypassPanel = uiToggleRow(PUtil, "Chrome.vs Bypass", M.bypassPanelOpen == true, function(on)
        if M.setBypassPanelOpen then
            M.setBypassPanelOpen(on)
        else
            M.bypassPanelOpen = on and true or false
            if on and M.buildChromeBypassUI then
                M.buildChromeBypassUI()
...
player.CharacterAdded:Connect(function(char)
    ...
    if M.antiAntiDesyncEnabled then M.startAntiAntiDesync() end
    if M.duelFaceEnabled then M.startDuelFace() end
...
function M.playIntro()
    ...
    local MUSIC_FILE = M.INTRO_MUSIC_FILE or "ChromeIntro_Music.mp3"
    local MUSIC_VOLUME = 0.75
    local BPM = 100
    local BEAT = 60 / BPM

    local IMAGE_ID = M.customBgAsset
    if not IMAGE_ID or IMAGE_ID == "" then
        local DEFAULT_BG = tonumber(M.customBgId) or tonumber(M.DEFAULT_BG_ID) or 78248012786524
        IMAGE_ID = "rbxassetid://" .. tostring(DEFAULT_BG)
    end

    for _, name in ipairs({"ShadowVSIntro", "ChromeDuelsIntro", "ChromeIntro"}) do
        pcall(function()
...
-- ============================================================
-- CHROME LAGGER V3 (Kill Hub style panel + open/close)
...
function M.buildKillLaggerPanel()
    -- REMOVED: Kill Lagger / Water.vs Lagger permanently disabled
    return
end
--[[ ORIGINAL buildKillLaggerPanel REMOVED 
    local UIS = game:GetService("UserInputService")
    local TweenService = game:GetService("TweenService")
    local RunService = game:GetService("RunService")
    ...
    titleLabel.Text = "CHROME.VS  ·  LAGGER"
...
-- Lagger / Ping / Bypass panels permanently removed
task.spawn(function()
    task.wait(0.35)
    pcall(function()
