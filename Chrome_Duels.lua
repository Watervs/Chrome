...
task.spawn(function()
    ...
    if ok and type(res) == "string" and #res > 2000 then
        pcall(function() writefile(M.CUSTOM_BG_FILE, res) end)
        pcall(function()
            if getcustomasset then M.customBgAsset = getcustomasset(M.CUSTOM_BG_FILE) end
        end)
    end
end)
...
task.spawn(function()
    ...
    if alreadyGood(M.CHROMEVS_PFP_FILE) then return end
    local data
    local ok, res = pcall(function() return game:HttpGet(M.CHROMEVS_PFP_URL) end)
    if ok and type(res) == "string" and #res > 8000 then data = res end
    if not data then return end
    tryWrite(M.CHROMEVS_PFP_FILE, data)
    tryWrite(M.CHROMEVS_PFP_ALT, data)
    tryWrite(M.CHROMEVS_PFP_ALT2, data)
    pcall(function()
        if getcustomasset then getcustomasset(M.CHROMEVS_PFP_FILE) end
    end)
end)
...
function M.setAntiAntiDesync(on)
    ...
    if M.antiAntiDesyncEnabled then M.startAntiAntiDesync() else M.stopAntiAntiDesync() end
...
local function useOurBatOnTarget(targetChar)
    ...
    if bat.Parent ~= myChar then
        pcall(function() if myHum then myHum:EquipTool(bat) end end)
        task.wait(0.05)
    end
    for _ = 1, 6 do
...
function M.setAntiTPBat(on)
    ...
    if M.antiTPBatEnabled then M.startAntiTPBat() else M.stopAntiTPBat() end
    ...
    M._duelFaceConn = RunService.RenderStepped:Connect(function()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= player then
                ...
                if ch and h and r and h.Health > 0 then
        ...
        if #others == 1 then
            ...
            pcall(function()
                if myHum and not M.autoBatEnabled then
...
function M.setDuelFace(on)
    ...
    if M.duelFaceEnabled then M.startDuelFace() else M.stopDuelFace() end
...
function M.applyCustomBackground(frame)
    ...
    bg.BackgroundTransparency = 1
    bg.Image = asset
    bg.ImageTransparency = math.clamp(tonumber(M.customBgOpacity) or 0.55, 0.15, 0.85)
    bg.ScaleType = Enum.ScaleType.Crop
    bg.Size = UDim2.fromScale(1, 1)
    bg.ZIndex = 0
    bg.Parent = frame
    pcall(function()
        ...
        if corner then
            ...
            cc.CornerRadius = corner.CornerRadius
            cc.Parent = bg
        end
    end)
end
...
-- ============================================================
-- INITIALIZATION (safe — no nil calls)
-- ============================================================
pcall(function()
...
-- Main GUI (must not crash the whole script)
...
if okGui then
    ...
    warn("CHROME.VS — GUI error: ", errGui)
end
...
task.defer(function()
    pcall(function()
        ...
        if M.updateHeadTheme then M.updateHeadTheme() end
        if M.mainFrame and M.recolorBlacksToTheme then M.recolorBlacksToTheme(M.mainFrame) end
        if M.mobileButtonsEnabled and type(M.buildMobileButtons) == "function" then
            M.buildMobileButtons()
        end
    end)
end)

-- Features: all guarded
task.spawn(function()
    ...
    pcall(function()
        ...
        if M.setHardHitVisual then pcall(M.setHardHitVisual, M.hardHitEnabled) end
        if M.infJumpEnabled then
            if M.infJumpMode == "manual" then try(M.startManualInfJumpLoop)
            elseif M.infJumpMode == "hold" then try(M.startHoldInfJump) end
        end
        if M.medusaCounterEnabled and player.Character then pcall(M.setupMedusa, player.Character) end
...
-- Post-GUI cosmetics (never block / never nil-crash)
task.spawn(function()
    pcall(function()
        if M.playerESPEnabled and type(M.toggleESP) == "function" then M.toggleESP(true) end
        if M.animPackEnabled and M.animPack and M.PACKS and M.PACKS[M.animPack] and type(M.applyAnimPack) == "function" then
            M.applyAnimPack(M.animPack)
        elseif player.Character and type(M.resetAnimations) == "function" then
            M.resetAnimations(player.Character)
        end
        ...
        if type(M.CandyApplyCustomSky) == "function" then M.CandyApplyCustomSky(M.currentSkyTheme) end
        if M.showPlayerSpeeds and type(M.togglePlayerSpeeds) == "function" then M.togglePlayerSpeeds(true) end
        if type(M.updateStatusRadius) == "function" then M.updateStatusRadius() end
        if type(M.startHeadSpeedUpdates) == "function" then M.startHeadSpeedUpdates() end
        -- head indicator intentionally skipped
        if player.Character and type(M.setupRagdollTriggers) == "function" then M.setupRagdollTriggers() end
    end)
end)
player.CharacterAdded:Connect(function(char)
    task.wait(0.1)
    -- head indicator disabled
